...
const OpenFundPanel: React.FC<OpenFundPanelProps> = ({
  selectedFunds,
  onAddToSelected,
  ...
  const columns = [
    ...
    },
    {
      ...
      render: (value: string) => value ? moment(value).format('YYYY-MM-DD') : '-',
    },
    {
    ...
    {
      title: '自定义', dataIndex: '自定义', key: '自定义', width: 100,
      sorter: numberSorter('自定义'), render: (v: number) => `${v?.toFixed(2)}%`,
      ...
      <TabPane tab="列表" key="all">
        <Card style={{ marginBottom: 16 }}>
          <div style={{ display: 'flex', gap: 16, alignItems: 'center' }}>
            <span>基金类型：</span>
            <Select style={{ width: 200 }} value={fundType} onChange={setFundType} options={fundTypeOptions} />
            <Button type="primary" onClick={fetchFundData} loading={loading}>搜索</Button>
          </div>
        </Card>
