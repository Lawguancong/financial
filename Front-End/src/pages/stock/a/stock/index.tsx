...
const handleNavigateToDetail = (code: string) => {
  const startDate = '20050101';
  // window.open(`/stock/a/stock/detail?symbol=${code}&period=daily&start_date=${startDate}&end_date=${moment().format('YYYYMMDD')}&adjust=qfq`, '_blank');
  window.open(`/stock/a/stock/detail?symbol=${code}&period=daily&end_date=${moment().format('YYYYMMDD')}`, '_blank');
...
const columns = [
  ...
  },
  {
    ...
    // render: (code: string) => (
    //   <Link 
    //     onClick={() => handleNavigateToDetail(code)}
  ...
  },
  {
    ...
    render: (name: string, record: StockData) => (
      <Link
        onClick={() => handleNavigateToDetail(record.代码)}
        style={{ cursor: 'pointer', color: '#1890ff' }}
      >
        {name}
      </Link>
